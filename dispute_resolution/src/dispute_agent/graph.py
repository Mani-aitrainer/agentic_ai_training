...
# ---------------------------------------------------------------------------
# Original single-agent linear pipeline (kept for teaching contrast).
# ---------------------------------------------------------------------------
def build_linear_graph(deps: Dependencies | None = None, *, checkpointer=None):
    """Assemble the original linear intake -> classify -> fraud_check -> decide
    -> (human_approval) -> refund/reject/escalate pipeline."""
    if deps is None:
    ...
    g.add_node("fraud_check", nodes["fraud_check"])
    ...
    g.add_conditional_edges(
        ...
        {"decide": "decide", "fraud_check": "fraud_check", "escalate": "escalate"},
    )
